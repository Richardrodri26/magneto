...
interface IHolidays { date: Date | string; name: string; nextMonday?: boolean; static: boolean }
 ...
 */
function applyTwoDigits(number: number) {
 ...
 */
function sumDay(stringDate: string, dayToSum: number) {
 ...
 * @returns {Array} Array con todos los festivos del año
 */
export function getHolidaysByYearInterval(initialYear: number, finalYear: number) {
	let holidaysArray = [];
	//Obtiene el domingo de pascua para calcular los días litúrgicos
	for (let i = initialYear; i <= finalYear; i++) {
		let year = {
			year : i,
			holidays: [] as IHolidays[]
		};
